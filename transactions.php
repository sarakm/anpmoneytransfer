...
$fee=htmlspecialchars($_REQUEST['fee']);
...
{ //  echo "finding userid";
	...
	$result = mysql_query($sql) or die(mysql_error());
	  ...
	  else
	  {
	   ...
	   $q="select transactionid,userid,receiver_firstname,receiver_lastname,date_submitted,amount_to_receive from transactions where transactionid=(select max(transactionid) from transactions)";
	  ...
	  {  
	    // echo" 1";
	     $row=mysql_fetch_array($res);
	     $today=date('Y-m-d');
			 if($sender_id==$row['userid']&& $receiver_firstname==$row['receiver_firstname']&& $receiver_lastname==$row['receiver_lastname']&& $cvt==$row['amount_to_receive']&& $row['date_submitted']==$today)
			 { 
		 $_SESSION['msg']="Duplicate Transaction values received.\nPlease change some values if you really want to send another transaction again!";
		...
		'$receiver_gender','$receiver_address1','$receiver_address2','$receiver_city','$receiver_province','$receiver_country','$receiver_postalcode','$receiver_phone1','$receiver_phone2','$receiver_email','$receiver_pid','$staff_photoid',CURDATE(),'$grt','$cvt','$agentid','$reason','$status','$exnote','$currency','$booker','$trans','$trans_time','$amt','$fee','$rate')";
			  ...
			  {			//echo $insert_id; 
					// $_SESSION['staffid']=$insert_id;
					 ...
					 header("location:trans_receipt.php?trans_id=".$insert_id);
