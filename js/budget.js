...
function checkBudget()
{
	...
	var budget = document.getElementById("addbudget").value;
	...
	if(budget > cbudget)
	{
		...
		document.getElementById("addbudget").style.background="red";
		return false;
	}
	else
	{
		document.getElementById("bmessage").innerHTML="";
		document.getElementById("addbudget").style.background="";
	}
...
function updateBudget(str){
	...
	window.setTimeout(window.location.reload("true"),99000000);
...
function updateMBudget()
{
	var tid="admin";
	var where="level";
	var col="budget";
	var tbl="staff";
	val=escape(document.getElementById("mbudget").value);
	
	
var dbRequest=getXmlHttpRequestObject();
	if(dbRequest.readyState == 4 || dbRequest.readyState == 0)
		{
			var str='updateMasterDB.php?tid=' + tid + '&col='+ col + '&val=' + val + '&tbl=' + tbl +'&where=' + where;
			confirm("updating database!");
		dbRequest.open("GET", str, true);
