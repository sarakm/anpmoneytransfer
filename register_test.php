...
{
//echo "photoid=  ".$_FILES["photoid"];
if((($_FILES["photoid"]["type"] == "image/gif")||($_FILES["photoid"]["type"] == "image/jpg")|| ($_FILES["photoid"]["type"] == "image/jpeg")|| ($_FILES["photoid"]["type"] == "image/pjpeg")))  {
  if ($_FILES["photoid"]["error"] > 0)
    {
    echo "Return Code: " . $_FILES["photoid"]["error"] . "<br />";
    }
  else
    {
   echo "Upload: " . $_FILES["photoid"]["name"] . "<br />";
    echo "Type: " . $_FILES["photoid"]["type"] . "<br />";
    echo "Size: " . ($_FILES["photoid"]["size"] / 1024) . " Kb<br />";
    echo "Temp file: " . $_FILES["photoid"]["tmp_name"] . "<br />";

    if (file_exists("uploads/" . $_FILES["photoid"]["name"]))
      {
      $msg= $_FILES["photoid"]["name"] . " already exists. ";
	  header("location:register.php?msg=".$msg);
	  exit;
      }
    else
      {
      $arr=move_uploaded_file($_FILES["photoid"]["tmp_name"],"uploads/" . $_FILES["photoid"]["name"]);
	  $filename="uploads/" . $_FILES["photoid"]["name"];
...
$s = "SELECT * FROM users where ((UPPER(firstname)=UPPER('$firstname')) && (UPPER(middlename)=UPPER('$middlename')&&(UPPER(lastname)=UPPER('$lastname')))";
...
$sql="INSERT INTO USERS(firstname,middlename,lastname,gender,address1,address2,city,province,country,postalcode,phone1,phone2,email,PID_DLN,identification,date)
      VALUES('$firstname','$middlename','$lastname','$gender','$address1','$address2','$city','$province','$country','$postalcode','$phone1','$phone2','$email','$pid','$filename',CURDATE())";
...
<body>
<div id='msg' style='color:#900' align="center"><?php if($_REQUEST['msg']!="") echo $_REQUEST['msg']; ?></div>
<FORM  ACTION="register.php" METHOD="POST">
  <table width="656" class="MYTABLE" ALIGN="CENTER">
    ...
    <tr class="MYTABLE">
      ...
      <td width="32"></td>
      <td width="291"><input type="text" name="custId" id="custId" value=""  onFocus="this.disabled=true" AUTOCOMPLETE="off"/></td>
    </tr>
      <tr class="MYTABLE">
